...
import { Form, Button, Col, InputGroup, ButtonGroup, Row, ToggleButton, ToggleButtonGroup, Container, OverlayTrigger, Tooltip, Alert, Dropdown } from "react-bootstrap";
...
function Insert_student_request_Form({
    ...
    useEffect(() => {
        const fetchOptionsData = async () => {
            try {
                ...
                const myApplicationsData = await ApplicationsAPI.getMyApplications() || [];
                const my_accepted_apps = myApplicationsData.filter(f=> f.Status == "Accepted")

                if (my_accepted_apps.length > 0){
                    setmyApplications(
                        my_accepted_apps
                        )
                }
                    ...
                    setmyApplications(
                            "There is not accepted application!"
                        )
                }
    ...
    const handle_selected_proposal = (eventKey) => {
    
        const the_application  = myApplications.filter(mA => mA.Application_Id == eventKey)

        setselectedApplication(the_application)
        setenableSelection(true)

        setProposalData(() => ({
    
            [ProposalFields.Title]: the_application[0].Proposal.Title,
            [ProposalFields.Description]: the_application[0].Proposal.Description,
            [ProposalFields.Type]: the_application[0].Proposal.Type,
            [ProposalFields.Required_Knowledge]: the_application[0].Proposal.Required_Knowledge,
            [ProposalFields.Notes]: the_application[0].Proposal.Notes,
            [ProposalFields.Expiration]: the_application[0].Proposal.Expiration,
            [ProposalFields.cosupervisors]: the_application[0].Proposal.cosupervisors,
            [ProposalFields.externalCosupervisors]: the_application[0].Proposal.externalCosupervisors,
            [ProposalFields.degrees]: the_application[0].Proposal.degrees,
            [ProposalFields.groups]: the_application[0].Proposal.groups,
            [ProposalFields.keywords]: the_application[0].Proposal.keywords,
            [ProposalFields.Supervisor]: the_application[0].Proposal.Supervisor,
            [ProposalFields.Level]: the_application[0].Proposal.Level
          }))        
    };  
    ...
    const handleInsertOrUpdateThesis = async (event) => {
        event.preventDefault();

        let data_to_be_sent = {}
        data_to_be_sent.Title = proposalData.Title
        data_to_be_sent.Student_Id = user.id
        data_to_be_sent.Supervisor_Id = selectedApplication[0].Proposal.Supervisor.Id
        data_to_be_sent.Description = proposalData.Description
        data_to_be_sent.Status = selectedApplication[0].Status
        data_to_be_sent.cosupervisors = selectedApplication[0].Proposal.cosupervisors
        
        ThesisAPI.addOrUpdateThesisRequest(data_to_be_sent).then((response) => {
            ...
            if (response.status === 200) {
                ...
                sweetalert({
                    title: "Request Not Is Sent, There Is a Problem",
                    icon: "error",
                    button: "Ok",
                });
            }
                    ...
                    <Row>
                        ...
                        <Dropdown>
                            <Dropdown.Toggle  id="dropdown-basic">
                                Select an approved proposal to start
                            </Dropdown.Toggle>
                            <Dropdown.Menu>
                                {
                                    typeof myApplications !== "string" ?(
                                        myApplications.map(mA =>  
                                                <Dropdown.Item 
                                                key={mA.Application_Id}
                                                eventKey={mA.Application_Id}
                                                onClick={() => handle_selected_proposal(mA.Application_Id)}>{mA.Proposal.Title}
                                                </Dropdown.Item>)
                                    ):(
                                    <Dropdown.Item style={{ cursor: "not-allowed" }}>{myApplications}</Dropdown.Item>
                                    )
                                }
                            </Dropdown.Menu>
                        </Dropdown>        
                    ...
                    <div className="actions-section">
                        {enableSelection &&
                            <>
                                ...
                                <Button id="save-button" onClick={handleInsertOrUpdateThesis}>
                                    Save
                                </Button>
                            </>
                        }
...
export default Insert_student_request_Form;
